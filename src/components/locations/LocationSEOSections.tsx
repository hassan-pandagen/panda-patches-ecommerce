...
export default function LocationSEOSections({ slug }: { slug: string }) {
    ...
    "patches-for-hats": [
      {
        ...
        content: (
          <div className="space-y-4">
            <p className="text-[1rem] leading-[1.8] text-gray-600 mb-6">Not every hat takes a patch the same way. Here is how each hat style works with custom patches so you order the right backing the first time.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="border border-gray-200 rounded-[12px] p-4 bg-white">
                  <p className="font-black text-panda-dark text-[0.9375rem] mb-1">{item.hat}</p>
                  <p className="text-[0.8125rem] text-gray-500 mb-1"><span className="font-semibold text-gray-700">Panel:</span> {item.panel}</p>
                  <p className="text-[0.8125rem] text-gray-500 mb-1"><span className="font-semibold text-gray-700">Best backing:</span> {item.backing}</p>
                  <p className="text-[0.8125rem] text-panda-green font-semibold"><span className="text-gray-700 font-semibold">Best patch type:</span> {item.best}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      },
      {
        ...
        content: (
          <div className="bg-panda-green/5 border border-panda-green/20 rounded-[16px] p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <p className="text-[0.9375rem] text-gray-700 font-medium leading-[1.6] max-w-[520px]">Not sure which patch type fits your hat? Get a digital mockup in 12 to 24 hours and see it before you order.</p>
            <a href="#quote-form" className="flex-shrink-0 bg-panda-green text-white font-black text-[0.875rem] px-6 py-3 rounded-full hover:bg-panda-dark transition-colors whitespace-nowrap">Get Your 12-24h Mockup</a>
          </div>
        )
      },
      {
        ...
        content: (
          <div>
            <p className="text-[1rem] leading-[1.8] text-gray-600 mb-6">Size is the most common mistake with hat patches. Too large and the patch overhangs the panel. Too small and it gets lost. Use this guide before you order.</p>
            ...
            <p className="text-[0.8125rem] text-gray-400 mt-3">Not sure which size fits your hat? Send us your hat and design and we will confirm the right dimensions before production.</p>
          </div>
        )
      },
      {
        ...
        content: (
          <div className="bg-blue-50 border border-blue-100 rounded-[16px] p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <p className="text-[0.9375rem] text-gray-700 font-medium leading-[1.6] max-w-[520px]">Need help choosing the right size? Our design team will recommend the perfect dimensions for your hat style.</p>
            <Link href="/contact" className="flex-shrink-0 bg-panda-dark text-white font-black text-[0.875rem] px-6 py-3 rounded-full hover:bg-panda-green transition-colors whitespace-nowrap">Ask Our Design Team</Link>
          </div>
        )
      },
      {
            ...
            {[
              ...
              <div key={i} className="border border-gray-200 rounded-[12px] p-5 bg-white hover:border-panda-green transition-colors">
                <div className="flex items-center justify-between mb-2">
                  <Link href={item.href} className="font-black text-panda-dark text-[1rem] hover:text-panda-green">{item.type}</Link>
                  <span className="text-[0.6875rem] font-bold text-panda-green bg-panda-light px-2.5 py-1 rounded-full">{item.tag}</span>
                </div>
                <p className="text-[0.75rem] text-gray-500 mb-2"><span className="font-semibold text-gray-700">Best for:</span> {item.best}</p>
                <p className="text-[0.75rem] text-gray-500 mb-2"><span className="font-semibold text-gray-700">Pros:</span> {item.pros}</p>
                <p className="text-[0.75rem] text-gray-500 mb-3"><span className="font-semibold text-gray-700">Cons:</span> {item.cons}</p>
                <p className="text-[0.8125rem] font-black text-panda-dark">{item.price} <span className="font-medium text-gray-500">(2x2 inches, 1,000 pcs)</span></p>
              </div>
            ))}
          </div>
        )
      },
      {
        ...
        content: (
          <div className="bg-amber-50 border border-amber-100 rounded-[16px] p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <p className="text-[0.9375rem] text-gray-700 font-medium leading-[1.6] max-w-[520px]">Want to see the difference in person? Order a free sample patch before your full order.</p>
            <Link href="/contact" className="flex-shrink-0 bg-panda-dark text-white font-black text-[0.875rem] px-6 py-3 rounded-full hover:bg-panda-green transition-colors whitespace-nowrap">Request a Free Sample</Link>
          </div>
        )
      },
      {
        ...
        content: (
          <div>
            <p className="text-[1rem] leading-[1.8] text-gray-600 mb-6">The backing determines how the patch attaches to your hat. Choose the wrong one and the patch will lift, shift, or not adhere at all.</p>
            <div className="overflow-x-auto rounded-[12px] border border-gray-200">
              <table className="w-full text-[0.875rem] text-left">
                <thead className="bg-panda-dark text-white">
                  <tr>
            ...
            <p className="text-[0.8125rem] text-gray-400 mt-3">Not sure which backing to choose? Tell us your hat material when requesting your quote and we will recommend the right option.</p>
          </div>
        )
      },
      {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="bg-white rounded-[16px] p-6 border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col gap-3">
                ...
                <h3 className="font-black text-panda-dark text-[1rem]">{item.title}</h3>
                <p className="text-[0.875rem] text-gray-600 leading-[1.7]">{item.body}</p>
              </div>
            ))}
          </div>
        )
      },
      {
        ...
        content: (
          <div>
            <p className="text-[1rem] leading-[1.8] text-gray-600 mb-6">All hat patch orders include a digital mockup in 12 to 24 hours, unlimited revisions, free worldwide shipping, and production only after your approval.</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              {[
                ...
                >
                  <p className="text-panda-yellow font-black text-[0.875rem] mb-1">{tier.qty}</p>
                  <p className="text-white font-black text-[1.125rem] mb-1">{tier.from}</p>
                  <p className="text-gray-400 text-[0.75rem]">{tier.per}</p>
                </a>
              ))}
            </div>
            <p className="text-[0.875rem] text-gray-500">Prices shown for embroidered patches under 4 inches. <Link href="/offers" className="text-panda-green font-semibold hover:underline">View all hat patch packages on our Offers page</Link> or <Link href="/contact" className="text-panda-green font-semibold hover:underline">get a free custom quote</Link> for larger quantities or other patch types.</p>
          </div>
        )
      },
      {
      ...
      },
      {
        ...
        content: (
          <div className="space-y-5 text-[1rem] leading-[1.8] text-gray-600">
            <p>
  ...
  return (
    <section className="py-8 md:py-10 bg-white border-t border-gray-100">
      <div className="container mx-auto px-6 max-w-[53.75rem] space-y-10">
