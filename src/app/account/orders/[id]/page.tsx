    ...
    <main className="min-h-screen bg-[#F9FAF5]">
      ...
      <section className="w-full pt-10 md:pt-14 pb-10 md:pb-16">
        <div className="container mx-auto px-6 max-w-[62.5rem]">
          ...
          <Link href="/account/orders" className="inline-flex items-center gap-1.5 text-[0.8125rem] text-gray-500 hover:text-panda-dark mb-4 transition-colors">
            <ChevronLeft size={14} /> Back to orders
          </Link>

          <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
            <div>
              <p className="text-[0.75rem] font-black uppercase tracking-[2px] text-panda-green mb-2">Order Details</p>
              <h1 className="text-[1.75rem] md:text-[2.25rem] font-black text-panda-dark leading-[1.1] tracking-tight">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white border border-gray-200 rounded-[20px] p-6 md:p-7">
                <h2 className="text-[1rem] font-black text-panda-dark uppercase tracking-wider mb-5">Order Status</h2>
                    ...
                    return (
                      <li key={i} className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className={`text-[0.875rem] font-bold ${step.done || step.current ? "text-panda-dark" : "text-gray-400"}`}>
                            {step.label}
                          </p>
                          {step.date && (
                            <p className="text-[0.75rem] text-gray-500 mt-0.5">{formatDate(step.date)}</p>
                          )}
                        </div>
                      </li>
                    );
              ...
              {/* Order summary */}
              <div className="bg-white border border-gray-200 rounded-[20px] p-6 md:p-7">
                <h2 className="text-[1rem] font-black text-panda-dark uppercase tracking-wider mb-5">Order Summary</h2>
                <dl className="grid grid-cols-3 gap-y-3 text-[0.875rem]">
                  {order.design_name && (
                    <>
                            ...
                            return (
                              ...
                              >
                                {isImage ? (
                                  ...
                                  <span className="flex w-[88px] h-[88px] items-center justify-center rounded-[10px] border border-gray-200 bg-gray-50 text-[0.6875rem] font-bold text-gray-500 uppercase tracking-wider group-hover:border-panda-green group-hover:text-panda-dark transition-colors">
                                    {(lower.match(/\.([a-z0-9]{2,4})(\?|$)/) || [, "file"])[1]}
                                  </span>
                                )}
                                <span className="inline-flex items-center gap-1 text-[0.6875rem] font-semibold text-panda-green group-hover:underline">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-panda-dark text-white rounded-[20px] p-6 md:p-7">
                <p className="text-[0.6875rem] font-bold text-gray-400 uppercase tracking-wider mb-2">Total Paid</p>
                <p className="text-[2.25rem] font-black text-panda-yellow leading-none">
                  ${(order.amount_paid || 0).toFixed(2)}
                </p>
                {order.paid_at && (
                  <p className="text-[0.75rem] text-gray-400 mt-3">Paid {formatDate(order.paid_at)}</p>
                )}
              </div>

              <div className="bg-white border border-gray-200 rounded-[20px] p-6 md:p-7 space-y-3">
                <h2 className="text-[0.875rem] font-black text-panda-dark uppercase tracking-wider mb-2">Order Again</h2>

                <form action="/api/account/reorder" method="post">
                  <input type="hidden" name="orderId" value={order.id} />
                  <button
                    type="submit"
                    className="w-full bg-panda-dark text-panda-yellow font-black text-[0.875rem] py-3 rounded-full uppercase tracking-wider hover:scale-[1.02] transition-transform"
                  >
                    Reorder &amp; Pay Now
                  </button>
                </form>
                ...
                <Link
                  ...
                  className="block w-full text-center bg-white border-2 border-panda-dark text-panda-dark font-black text-[0.875rem] py-3 rounded-full uppercase tracking-wider hover:bg-panda-dark hover:text-panda-yellow transition-colors"
                >
                  Get a New Quote
                </Link>
                ...
                {isWithinLoyaltyWindow && (
                  <div className="bg-panda-green/10 border border-panda-green/30 rounded-[10px] px-3 py-2.5 text-center">
                    <p className="text-[0.6875rem] font-bold text-panda-dark leading-[1.5]">
                ...
                <p className="text-[0.6875rem] text-gray-400 text-center leading-[1.6] pt-2">
              ...
              <div className="bg-white border border-gray-200 rounded-[20px] p-6 md:p-7">
                <h2 className="text-[0.875rem] font-black text-panda-dark uppercase tracking-wider mb-3">Need Help?</h2>
                <p className="text-[0.8125rem] text-gray-500 leading-[1.7] mb-3">
                ...
                <p className="text-[0.8125rem] text-gray-500 leading-[1.7]">
