        ...
        {formData.artworkUrl ? (
          <div className="flex items-center justify-between bg-green-50 border border-green-300 rounded-xl px-4 py-3">
            ...
            <button
              type="button"
              onClick={clearFile}
              className="ml-3 w-11 h-11 flex items-center justify-center rounded-full bg-red-100 text-red-500 hover:bg-red-200 transition-colors shrink-0"
...
// ─── Order Flow wrapper ────────────────────────────────────────────────────────
// Everything a customer sees after clicking "Order Now" on an offer card — the
// inline expanding form header, step progress bar, and steps 1-4. Code-split out
// of OffersClient (dynamic import, ssr:false) so this ~600-line, form-heavy
// bundle only downloads/hydrates on that click instead of on every page load
// (audit P2-1 — this was the largest remaining chunk on the path to 90+ perf).

export default function OrderFlow({
  offer, step, setStep, total, formData, setFormData,
  fileName, uploading, handleFileUpload, clearFile,
  loading, error, onStripe, onClose,
}: {
  offer: SelectedOffer;
  step: number;
  ...
  return (
    <div className="mt-8 border-2 border-[#051C05] rounded-2xl overflow-hidden">
      {/* Form header */}
      <div className="bg-[#051C05] px-5 py-4 flex items-center justify-between">
        <div>
          ...
          <div className="font-bold text-[#DFFF00] text-sm">{offer.type} — {offer.packName} ({offer.qty} pcs)</div>
        </div>
      ...
      <div className="px-5 pb-6">
        ...
        {step === 4 && (
          <Step5
            offer={offer} formData={formData} total={total}
