    ...
    <div className="w-full py-8 bg-gray-50 border-b border-gray-100">
      <div className="container mx-auto px-4 max-w-[75rem]">
        <div className="text-center mb-6">
          <h2 className="text-[1rem] font-black text-gray-800">What Our Customers Say</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {REVIEWS.map((r, i) => (
            <div key={i} className="bg-white rounded-[14px] p-5 border border-gray-100 shadow-sm">
              <p className="text-[0.8125rem] text-gray-600 leading-[1.7] mb-4 line-clamp-3">&ldquo;{r.quote}&rdquo;</p>
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full ${r.color} flex items-center justify-center text-white text-[0.6875rem] font-black flex-shrink-0`}>
                  {r.initials}
                </div>
                <div>
                  <p className="text-[0.8125rem] font-bold text-gray-800">{r.name}</p>
                  <p className="text-[0.6875rem] text-gray-400">{r.location}</p>
                </div>
              </div>
            </div>
          ))}
