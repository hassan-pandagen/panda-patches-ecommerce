import Image from "next/image";
import { urlFor } from "@/lib/sanity";
import { Shield, Clock, Gift, Award } from "lucide-react";
import DesignServiceForm from "./DesignServiceForm";

type ServiceType = "digitizing" | "vector";

interface DesignServiceHeroProps {
  serviceType: ServiceType;
  heroImage?: string | null;
  trustBadges?: Array<{ url: string; alt: string }>;
  heading: string;
  subheading: string;
  description: string;
}

export default function DesignServiceHero({
  serviceType,
  heroImage,
  trustBadges = [],
  heading,
  subheading,
  description,
}: DesignServiceHeroProps) {
  return (
    <section id="hero" className="w-full pt-6 md:pt-8 pb-6 md:pb-10 bg-white scroll-mt-24">
      <div className="container mx-auto px-4 md:px-6 max-w-[75rem]">
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
          {/* LEFT - Content */}
          <div className="flex-1 text-center lg:text-left">
            {/* H1 */}
            <h1 className="text-[1.75rem] md:text-[2.625rem] lg:text-[3rem] font-black text-panda-dark leading-[1.1] tracking-tight mb-4">
              {heading}
            </h1>

            {/* Subheadline */}
            <p className="text-[1rem] md:text-[1.1875rem] font-bold text-panda-dark leading-[1.3] mb-3 max-w-[560px] mx-auto lg:mx-0">
              {subheading}
            </p>

            {/* Description */}
            <p className="text-[0.9375rem] md:text-[1.0625rem] text-gray-600 leading-[1.6] font-medium mb-5 max-w-[560px] mx-auto lg:mx-0">
              {description}
            </p>

            {/* Trust Bar - Compact */}
            <div className="grid grid-cols-4 gap-2 max-w-[480px] mx-auto lg:mx-0 mb-6">
              {[
                { icon: Award, label: "Free", sub: "With Order" },
                { icon: Gift, label: "Free", sub: "Trial Job" },
                { icon: Clock, label: "12-24h", sub: "Mockup" },
                { icon: Shield, label: "No Setup", sub: "Fees" },
              ].map((item, idx) => (
                <div
                  key={idx}
                  className="flex flex-col items-center text-center bg-panda-light rounded-[10px] py-2.5 px-2 border border-gray-100"
                >
                  <item.icon className="w-4 h-4 md:w-5 md:h-5 text-panda-green mb-1" strokeWidth={2.5} />
                  <span className="text-[0.6875rem] md:text-[0.8125rem] font-bold text-panda-dark leading-tight">
                    {item.label}
                  </span>
                  <span className="text-[0.5625rem] md:text-[0.6875rem] text-gray-500 font-medium">{item.sub}</span>
                </div>
              ))}
            </div>

            {/* Trust Badge Strip */}
            {trustBadges.length > 0 && (
              <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 max-w-[480px] mx-auto lg:mx-0 mb-6">
                {trustBadges
                  .filter((badge) => badge?.url)
                  .map((badge, idx) => (
                    <div key={idx} className="relative h-[44px] w-[96px] flex-shrink-0">
                      <Image
                        src={urlFor(badge.url).width(200).format("webp").quality(80).url()}
                        alt={badge.alt || "Trust badge | Panda Patches"}
                        fill
                        className="object-contain"
                        sizes="96px"
                      />
                    </div>
                  ))}
              </div>
            )}

            {/* Hero Image */}
            {(() => {
              const hasSanityImage = !!heroImage;
              const imgSrc = hasSanityImage
                ? urlFor(heroImage).width(1040).height(640).quality(85).auto("format").url()
                : "/assets/hero-product.webp";
              return (
                <div className="relative w-full max-w-[520px] h-[200px] md:h-[320px] mx-auto lg:mx-0">
                  <Image
                    src={imgSrc}
                    alt={
                      serviceType === "digitizing"
                        ? "Embroidery digitizing service for custom patches"
                        : "Vector conversion service for custom patches"
                    }
                    fill
                    className="object-contain object-center lg:object-left"
                    priority
                    sizes="(max-width: 768px) 100vw, 520px"
                  />
                  {/* 1 Million Badge */}
                  <div className="hidden md:flex absolute bottom-6 right-6 bg-white shadow-xl rounded-xl p-2.5 items-center gap-2.5 border border-gray-100">
                    <div className="bg-[#E4EFE0] rounded-full p-1.5">✅</div>
                    <div>
                      <p className="text-[0.625rem] text-gray-400 uppercase font-bold tracking-wider">
                        Patches Delivered
                      </p>
                      <p className="font-black text-panda-dark text-base">1 Million +</p>
                    </div>
                  </div>
                </div>
              );
            })()}
          </div>

          {/* RIGHT - Design Service Form */}
          <div className="w-full lg:w-[520px] flex-shrink-0 lg:-mt-2">
            <DesignServiceForm serviceType={serviceType} />
          </div>
        </div>
      </div>
    </section>
  );
}
