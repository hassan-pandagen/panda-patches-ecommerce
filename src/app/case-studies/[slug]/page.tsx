...
export default async function CaseStudyDetailPage({
  ...
  return (
    <main className="min-h-screen bg-white">
          ...
          <div
            className={
              hasHero
                ...
                : "max-w-[51.25rem] mx-auto px-6 pt-24 pb-14"
            }
          >
            <div className={hasHero ? "max-w-[40rem]" : ""}>
              ...
              {cs.tags && cs.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-5">
                  {cs.tags.map((t) => (
                    <span
                      key={t}
                      className="text-[0.6875rem] font-bold uppercase tracking-wide text-panda-yellow bg-panda-yellow/15 px-2.5 py-1 rounded-full"
                    >
                      {t}
                    </span>
                  ))}
              ...
              <p className="text-[0.8125rem] font-bold uppercase tracking-[1.5px] text-panda-yellow mb-3">
                {cs.clientUrl ? (
                  <a
                    href={cs.clientUrl}
                    target="_blank"
              ...
              <h1 className="text-[1.875rem] md:text-[2.625rem] font-black leading-[1.1] tracking-tight mb-5">
                {cs.title}
              </h1>
              <p className="speakable-summary text-[1rem] md:text-[1.125rem] text-gray-300 leading-[1.7]">
                {cs.summary}
              </p>

              {/* Meta row */}
              <div className="flex flex-wrap gap-x-8 gap-y-3 mt-8 pt-7 border-t border-white/10 text-[0.875rem]">
                {cs.event && (
                  <div>
                    <div className="text-gray-500 text-[0.6875rem] uppercase tracking-wide font-bold mb-0.5">
                      Event
                    </div>
          ...
          <div className="max-w-[56.25rem] mx-auto grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            {cs.stats.map((st) => (
              <div key={st.label}>
                ...
                <div className="text-[0.8125rem] text-gray-500 font-medium mt-1.5">
                  {st.label}
                </div>
              </div>
            ))}
      ...
      {/* Body sections */}
      <article className="max-w-[47.5rem] mx-auto px-6 py-14 space-y-12">
        ...
        {cs.keyFacts && cs.keyFacts.length > 0 && (
          <section className="border border-gray-200 rounded-2xl p-6 md:p-7 bg-gray-50">
            <h2 className="text-[0.8125rem] font-black uppercase tracking-[1.5px] text-panda-green mb-4">
              At a glance
            </h2>
            <ul className="space-y-2.5">
              {cs.keyFacts.map((f, i) => (
                <li
                  key={i}
                  className="flex gap-2.5 text-[0.9375rem] text-gray-700 leading-[1.6]"
                >
        ...
        {cs.sections.map((sec) => (
          <section key={sec.heading}>
            ...
            <h2 className="text-[1.5rem] md:text-[1.875rem] font-black text-panda-dark leading-tight mb-4">
              {sec.heading}
            </h2>
            {paragraphs(sec.body).map((p, i) => (
              <p
                key={i}
                className="text-[1.0625rem] leading-[1.85] text-gray-700 mb-4"
              >
                {p}
              </p>
            ))}
            {sec.image?.src && (
              <figure className="mt-6">
                ...
                {(sec.image.caption || sec.image.credit) && (
                  <figcaption className="text-[0.75rem] text-gray-400 mt-2.5">
                    {sec.image.caption}
                    {sec.image.caption && sec.image.credit ? " " : ""}
        ...
        {cs.quote && (
          <blockquote className="border-l-4 border-panda-yellow bg-panda-light rounded-r-2xl px-7 py-6">
            <p className="text-[1.1875rem] md:text-[1.3125rem] font-semibold text-panda-dark leading-[1.6] mb-3">
      ...
      {cs.faqs && cs.faqs.length > 0 && (
        <section className="max-w-[47.5rem] mx-auto px-6 pb-16">
          <div className="w-10 h-1 bg-panda-yellow rounded-full mb-4" />
          <h2 className="text-[1.5rem] md:text-[1.875rem] font-black text-panda-dark mb-6">
            Frequently asked questions
          </h2>
          <div className="space-y-5">
            {cs.faqs.map((f, i) => (
              <div key={i} className="border-b border-gray-100 pb-5">
                <h3 className="text-[1.0625rem] font-bold text-panda-dark mb-2">
                  {f.question}
                </h3>
                <p className="text-[1rem] text-gray-700 leading-[1.7]">
                  {f.answer}
                </p>
              </div>
            ))}
      ...
      {cs.gallery && cs.gallery.length > 0 && (
        <section className="max-w-[62.5rem] mx-auto px-6 pb-16">
          <div className="w-10 h-1 bg-panda-yellow rounded-full mb-4" />
          <h2 className="text-[1.5rem] md:text-[1.875rem] font-black text-panda-dark mb-6">
            From the activation
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {cs.gallery.map((img) => (
              <figure key={img.src}>
                ...
                {img.credit && (
                  <figcaption className="text-[0.6875rem] text-gray-400 mt-1.5">
                    Photo: {img.credit}
                  </figcaption>
                )}
              </figure>
      ...
      <section className="max-w-[47.5rem] mx-auto px-6 pb-14">
        <div className="w-10 h-1 bg-panda-yellow rounded-full mb-4" />
        <h2 className="text-[1.25rem] md:text-[1.5rem] font-black text-panda-dark mb-5">
          Explore more
        </h2>
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <Link
              key={l.href}
              href={l.href}
              className="inline-flex items-center border border-gray-200 rounded-full px-4 py-2 text-[0.875rem] font-bold text-panda-dark hover:border-panda-green hover:text-panda-green transition-all"
            >
              {l.label}
            </Link>
          ))}
      ...
      <section className="bg-panda-dark text-white px-6 py-16">
        <div className="max-w-[45rem] mx-auto text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <Link
              href="/custom-patches"
              className="bg-panda-yellow text-panda-dark font-black text-[0.9375rem] px-8 py-3.5 rounded-full hover:bg-panda-green hover:text-white transition-all"
            >
              Start your patches
            </Link>
            <Link
              href="/case-studies"
              className="border border-white/30 text-white font-black text-[0.9375rem] px-8 py-3.5 rounded-full hover:border-panda-yellow hover:text-panda-yellow transition-all"
            >
