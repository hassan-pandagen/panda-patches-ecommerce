...
function CellResult({ value, win, lose }: { value: string; win?: boolean; lose?: boolean }) {
  ...
  return (
    <div className={`flex items-start gap-2 ${win ? 'text-panda-green font-bold' : lose ? 'text-gray-500' : 'text-gray-800'}`}>
      ...
      <span className="leading-relaxed text-[0.875rem] md:text-[0.9375rem]">{value}</span>
    </div>
  );
}
    ...
    <>
          ...
          <div className="text-center max-w-[51.25rem] mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center items-stretch sm:items-center">
              <Link
                href="/contact"
                prefetch={false}
                className="flex items-center justify-center gap-2 bg-[#DFFF00] text-[#051C05] font-bold text-[0.875rem] md:text-[0.9375rem] px-6 py-3.5 rounded-full hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 group whitespace-nowrap"
              >
              ...
              <Link
                href="/offers"
                prefetch={false}
                className="flex items-center justify-center gap-2 bg-[#051C05] text-[#DFFF00] font-bold text-[0.875rem] md:text-[0.9375rem] px-6 py-3.5 rounded-full hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 group whitespace-nowrap"
              >
      ...
      <section className="bg-[#FFFBEB] border-b border-amber-200 py-3 px-6">
        <div className="container mx-auto max-w-[56.25rem]">
          <p className="text-[0.75rem] text-amber-900 text-center leading-relaxed">
      ...
      <section className="w-full pb-8 md:pb-16 pt-10 md:pt-14 bg-[#F7F7F7]">
        <div className="container mx-auto px-6 flex flex-col items-center max-w-[68.75rem]">
          <h2 className="text-[1.5rem] md:text-[1.75rem] lg:text-[2rem] font-black text-center text-panda-dark uppercase tracking-wide mb-3">
            Why switch from {competitorName}?
          </h2>
          <p className="text-center text-gray-500 text-[0.875rem] md:text-[0.9375rem] font-medium mb-10 md:mb-14 max-w-[560px]">
            ...
            {threePillars.map((pillar, i) => {
              ...
              return (
                <div key={i} className="bg-white border border-gray-200 rounded-2xl p-7 hover:shadow-lg transition-shadow">
                  ...
                  <h3 className="text-[1.0625rem] md:text-[1.1875rem] font-extrabold text-panda-dark mb-3 leading-tight">{pillar.title}</h3>
                  <p className="text-[0.875rem] md:text-[0.9375rem] text-gray-600 leading-[1.6] font-medium">{pillar.body}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="bg-panda-dark py-14 md:py-20 px-6">
        <div className="container mx-auto max-w-[51.25rem] text-center">
          <blockquote className="text-[1.25rem] md:text-[1.75rem] text-white font-medium leading-[1.4] mb-6 [text-wrap:balance]">
            &ldquo;{primaryQuote.text}&rdquo;
          </blockquote>
          <p className="text-panda-yellow font-bold text-[0.875rem] md:text-[0.9375rem]">
            {primaryQuote.author}
      ...
      <section className="py-14 md:py-20 px-6 bg-white">
        <div className="container mx-auto max-w-[68.75rem]">
          <div className="text-center mb-10 md:mb-14">
            <h2 className="text-[1.5rem] md:text-[2rem] font-black text-panda-dark mb-3 leading-tight">
              Panda Patches vs {competitorName}
            </h2>
            <p className="text-gray-500 text-[0.875rem] md:text-[1rem] max-w-[37.5rem] mx-auto">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-black text-white">
                  <th className="text-left px-5 py-4 font-bold text-[0.75rem] uppercase tracking-wider">Feature</th>
                  <th className="text-left px-5 py-4 font-bold text-[0.75rem] uppercase tracking-wider bg-panda-green text-panda-dark">Panda Patches</th>
                  <th className="text-left px-5 py-4 font-bold text-[0.75rem] uppercase tracking-wider">{competitorName}</th>
                </tr>
              </thead>
              <tbody>
                {categoryOrder.map((category) => (
                  <>
                    <tr key={`cat-${category}`} className="bg-[#F7F7F7]">
                      <td colSpan={3} className="px-5 py-3 font-black text-[0.6875rem] uppercase tracking-[1.5px] text-panda-dark">
                        {category}
                      </td>
                    </tr>
                    {grouped[category].map((row, i) => (
                      <tr key={`${category}-${i}`} className="border-t border-gray-100 hover:bg-[#FAFAFA] transition-colors">
                        <td className="px-5 py-4 font-bold text-panda-dark align-top text-[0.875rem] md:text-[0.9375rem]">{row.feature}</td>
          ...
          <p className="text-[0.75rem] md:text-[0.8125rem] text-gray-500 mt-4 leading-relaxed max-w-[56.25rem] mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {pricingExamples.map((ex, i) => (
              <div key={i} className="bg-white rounded-2xl p-6 md:p-7 border border-gray-200 hover:shadow-md transition-shadow">
                <p className="text-[0.6875rem] font-black uppercase tracking-[1.5px] text-gray-500 mb-4">{ex.scenario}</p>
                <div className="grid grid-cols-2 gap-3">
                  <div className={`rounded-xl p-4 transition-colors ${ex.pandaWin ? 'bg-panda-green/10 border-2 border-panda-green' : 'bg-[#F7F7F7] border-2 border-transparent'}`}>
                    <p className="text-[0.6875rem] font-black uppercase tracking-wider text-panda-dark mb-1">Panda Patches</p>
                    <p className="text-[1.125rem] md:text-[1.25rem] font-black text-panda-dark leading-tight">{ex.panda}</p>
                    {ex.pandaWin && <p className="text-[0.6875rem] text-panda-green font-bold mt-2 uppercase tracking-wider">{`${CHECK} Better value`}</p>}
                  </div>
                  <div className="rounded-xl p-4 bg-[#F7F7F7]">
                    <p className="text-[0.6875rem] font-black uppercase tracking-wider text-gray-600 mb-1">{competitorName}</p>
                    <p className="text-[1.125rem] md:text-[1.25rem] font-black text-gray-700 leading-tight">{ex.competitor}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="container mx-auto max-w-[51.25rem]">
          <div className="bg-[#F7F7F7] border-l-4 border-gray-400 rounded-r-2xl p-7 md:p-9">
            <div className="flex items-start gap-3 mb-4">
              ...
              <h2 className="text-[1.375rem] md:text-[1.75rem] font-black text-panda-dark leading-tight">
                When {competitorName} might be the better choice
              </h2>
            </div>
            <p className="text-gray-700 leading-[1.7] text-[0.9375rem] md:text-[1rem] pl-9 font-medium">
              {whereCompetitorFits}
            </p>
          </div>
        </div>
        ...
        <div className="container mx-auto max-w-[56.25rem]">
          <div className="bg-white border-2 border-panda-green rounded-2xl p-8 md:p-12 shadow-sm text-center">
            <div className="inline-flex items-center gap-2 bg-panda-green/15 px-4 py-2 rounded-full mb-5">
              <Shield className="w-4 h-4 text-panda-green" />
              <span className="text-[0.6875rem] font-black uppercase tracking-[1.5px] text-panda-green">Money-Back Guarantee</span>
            </div>
            <h2 className="text-[1.5rem] md:text-[2.125rem] font-black text-panda-dark mb-4 leading-tight [text-wrap:balance]">
              We don&apos;t start production until you approve in writing
            </h2>
            <p className="text-gray-600 leading-[1.7] text-[0.9375rem] md:text-[1rem] max-w-[40rem] mx-auto mb-7 font-medium">
            ...
            <Link
              href="/contact"
              className="inline-block bg-panda-dark text-panda-yellow font-black text-[0.875rem] md:text-[0.9375rem] px-8 py-3.5 rounded-full hover:bg-panda-green hover:text-panda-dark transition-colors"
            >
          ...
          <div className="container mx-auto max-w-[51.25rem]">
            <div className="flex items-start gap-5">
              ...
              <div className="flex-1">
                <blockquote className="text-[1rem] md:text-[1.125rem] text-gray-800 leading-[1.7] italic mb-3 font-medium">
                  &ldquo;{secondaryQuote.text}&rdquo;
                </blockquote>
                <p className="text-[0.8125rem] md:text-[0.875rem] text-gray-600">
      ...
      <section className="py-14 md:py-20 px-6 bg-[#F7F7F7]">
        <div className="container mx-auto max-w-[51.25rem]">
          <div className="text-center mb-10 md:mb-14">
            <h2 className="text-[1.5rem] md:text-[2rem] font-black text-panda-dark mb-3 leading-tight uppercase tracking-wide">
              Common Questions
            </h2>
            <p className="text-gray-500 text-[0.875rem] md:text-[1rem]">
          ...
          <div className="space-y-4">
            {faqs.map((faq, i) => (
              <div key={i} className="bg-white border border-gray-200 rounded-2xl p-6 md:p-7 hover:shadow-md transition-shadow">
                <h3 className="text-[1rem] md:text-[1.125rem] font-black text-panda-dark mb-3 leading-tight">
                  {faq.question}
                </h3>
                <p className="text-gray-600 leading-[1.7] text-[0.875rem] md:text-[0.9375rem] font-medium">
                  {faq.answer}
                </p>
              </div>
            ))}
      ...
      <section className="relative w-full bg-panda-dark py-16 md:py-24 px-6 overflow-hidden">
        <div className="container mx-auto max-w-[51.25rem] text-center relative z-10">
          <h2 className="text-[1.75rem] md:text-[2.5rem] lg:text-[3rem] font-black text-white mb-4 leading-[1.1] [text-wrap:balance]">
            See what your patches will look like.
            <br />
            <span className="text-panda-yellow">Free, in 24 hours.</span>
          </h2>
          <p className="text-gray-300 leading-[1.6] mb-8 text-[0.9375rem] md:text-[1.0625rem] max-w-[37.5rem] mx-auto font-medium">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center items-stretch sm:items-center">
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 bg-[#051C05] text-[#DFFF00] font-bold text-[0.875rem] md:text-[0.9375rem] px-6 py-3.5 rounded-full hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 group whitespace-nowrap border-2 border-[#DFFF00]/30"
            >
